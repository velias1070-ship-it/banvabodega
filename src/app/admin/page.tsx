    ...
    <div className="app-admin">
      ...
      <div className="admin-layout">
        <nav className="admin-sidebar">
          ...
          {canAccessTab(auth.user, "qr_codes") && (
...
function AdminRecepciones({ refresh }: { refresh: () => void }) {
  ...
  const doResetLinea = async (lineaId: string) => {
    ...
    const ubicada = linea?.qty_ubicada || 0;
    const msg = ubicada > 0
      ? `Resetear esta línea a PENDIENTE?\n\nSe revertirán ${ubicada} uds del stock y se perderán conteos y ubicaciones.`
      : "Resetear esta línea a PENDIENTE? Se perderán conteos y ubicaciones.";
    if (!confirm(msg)) return;
    setLoading(true);
    await resetearLineaRecepcion(lineaId, selRec!.id!);
...
function PickingSessionDetail({ session: initialSession, onBack }: { session: DBPickingSession; onBack: () => void }) {
  ...
  const doDelete = async () => {
    if (!confirm("¿Eliminar esta sesión de picking completa?\n\nElimina el registro por completo (DELETE). Si querés mantener trazabilidad, usá 'Anular' en lugar de eliminar.")) return;
    await eliminarPicking(session.id!);
    onBack();
  };

  const doAnular = async () => {
    const motivo = prompt("¿Por qué anulás este envío? (opcional, queda en audit log)");
    if (motivo === null) return; // cancel
    if (!confirm("¿Anular esta sesión de picking?\n\nMantiene el registro (estado=ANULADA) pero libera reservas y re-sincroniza stock a ML.")) return;
    const ok = await anularPicking(session.id!, motivo || undefined);
    if (ok) onBack();
  ...
  const doDuplicar = async () => {
    ...
    if (newId) {
      if (errors.length > 0) {
        alert(`Envío duplicado OK con ${errors.length} advertencias:\n\n${errors.slice(0, 5).join("\n")}`);
      } else {
        showToast("Envío duplicado OK");
      }
      onBack();
    } else {
...
function Dashboard() {
  ...
  const [desgloseReservas, setDesgloseReservas] = useState<{flex:number;full:number}>({flex:0,full:0});
  ...
  const loadDashStock = useCallback(async () => {
    const [rows, desglose] = await Promise.all([fetchStockProyectado(), fetchDesgloseReservas()]);
  ...
  return (
    <div>
      ...
      <div className="admin-kpi-grid">
        ...
        <div className="kpi"><div className="kpi-label">Comprometido</div><div className="kpi-val" style={{color:"var(--amber)"}}>{dashStock.comprometido}</div><div className="kpi-sub" style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>{(desgloseReservas.flex > 0 || desgloseReservas.full > 0) && <span style={{fontSize:10,color:"var(--txt3)"}}><span style={{color:"var(--cyan)"}}>{desgloseReservas.flex}</span> Flex · <span style={{color:"var(--blue)"}}>{desgloseReservas.full}</span> Full</span>}<button onClick={runReconciliacion} disabled={reconciling} style={{background:"none",border:"1px solid var(--amber)",color:"var(--amber)",borderRadius:6,padding:"2px 8px",fontSize:11,cursor:"pointer",opacity:reconciling?0.5:1}}>{reconciling ? "Reconciliando..." : "Reconciliar"}</button></div></div>
                            ...
                            <td className="mono" style={{textAlign:"right",fontSize:11,color:reserved>0?"var(--amber)":"var(--txt3)",cursor:reserved>0?"pointer":"default",textDecoration:reserved>0?"underline":"none"}}
                              onClick={reserved > 0 ? async (e) => {
                                ...
                                try {
                                  const sb = (await import("@/lib/supabase")).getSupabase();
                                  if (sb) {
                                    // Query active shipments for this SKU (source of truth for reservations)
                                    const { data: shipItems } = await sb.from("ml_shipment_items").select("shipment_id, seller_sku, quantity, stock_deducted").eq("seller_sku", sku);
                                    if (shipItems && shipItems.length > 0) {
                                      const shipIds = shipItems.map(si => si.shipment_id);
                                      const { data: ships } = await sb.from("ml_shipments").select("shipment_id, status, handling_limit, receiver_name").in("shipment_id", shipIds).in("status", ["ready_to_ship", "shipped"]);
                                      const inferred = (ships || []).map(sh => {
                                        const items = shipItems.filter(si => si.shipment_id === sh.shipment_id);
                                        const qty = items.reduce((s, i) => s + i.quantity, 0);
                                        const deducted = items.every(i => i.stock_deducted);
                                        return { id: String(sh.shipment_id), sku, cantidad: qty, tipo: "venta_flex", referencia_tipo: "shipment", referencia_id: String(sh.shipment_id), estado: deducted ? "pickeado" : "active", created_at: sh.handling_limit || "", receiver_name: sh.receiver_name || "" };
                                      });
...
function Productos({ refresh }: { refresh: () => void }) {
  ...
  const save=async()=>{
    ...
    const newProd:Product={sku,name:form.name!,cat:form.cat||"Otros",prov:form.prov||"Otro",cost:form.cost||0,costAvg:s.products[sku]?.costAvg||form.cost||0,price:form.price||0,reorder:form.reorder||20,estadoSku:form.estadoSku||null,tamano:form.tamano||"",color:form.color||"",innerPack:form.innerPack??1};
    s.products[sku]=newProd;
    // Persistir sincrónicamente para detectar errores de schema/constraint
    // y mostrarlos al usuario (en lugar de tragarlos en el debounced flush).
    try {
      await saveProductAsync(newProd);
    } catch (err) {
      delete s.products[sku];
      alert("Error guardando producto: " + (err instanceof Error ? err.message : String(err)));
      return;
    }
  ...
  useEffect(() => {
    const doAutoSync = async () => {
      try {
        ...
        // Auto-create/update Flex picking session for today
