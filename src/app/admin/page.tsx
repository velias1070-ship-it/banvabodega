    ...
    for (const linea of newLineas) {
      ...
      for (const mov of movs) {
        ...
        if (mov.qty - yaUsado >= comp.unidades) {
          ...
          linea.estado = "PICKEADO";
          movUsado.set(usadoKey, yaUsado + comp.unidades);
          reparadas++;
          foundMov = true;
          break;
        }
...
function Configuracion({ refresh }: { refresh: () => void }) {
  const [configTab, setConfigTab] = useState<"general"|"posiciones"|"mapa"|"etiquetas"|"carga_stock"|"conteos"|"conciliador"|"diccionario">("general");
  ...
  return (
    <div>
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        {([["general","General","⚙️"],["diccionario","Diccionario","📖"],["posiciones","Posiciones","📍"],["mapa","Mapa Bodega","🗺️"],["etiquetas","Etiquetas","🖨️"],["carga_stock","Carga Stock","📥"],["conteos","Conteo Cíclico","📋"],["conciliador","Conciliador","🏦"]] as const).map(([key,label,icon])=>(
