-- Sprint 5.5.5 — Completar revert agotar en v_safety_stock.supplier_lt
-- Owner: Vicente Elías | 2026-05-04 PM | Tag: [batch:20260504-revert-supplier-lt][sprint:5.5.5]
-- Discovery: docs/sprints/sprint-5.5.5-revert-supplier-lt.md
-- Doctrina vigente: docs/policies/estados-sku.md (revisión 2026-05-04 PM)
-- Aplica Regla 2 inventory-policy: 4ta superficie del mismo filtro espejo.
--
-- Sprint 5.5.2 (AM) revirtió filtro en RPC refresh_sku_node_policy_from_templates.
-- Sprint 5.5.4 (PM) revirtió filtro en v_trend_detection.
-- Sprint 5.5.5 (PM, este) cierra el barrido en v_safety_stock.supplier_lt.
--
-- PROBLEMA:
-- v_safety_stock.CTE `supplier_lt` filtraba con WHERE estado_sku = 'activo' OR IS NULL.
-- Los 22 SKUs agotar caían fuera del CTE → LEFT JOIN externo devolvía NULL →
-- el motor usaba LT default 14 días en lugar de los 5 días del proveedor real
-- (Idetex). Inflación de safety_stock + reorder_point + qty_a_comprar.
--
-- CAMBIO:
-- supplier_lt ahora filtra con WHERE estado_sku IS DISTINCT FROM 'descontinuado'
-- (alineado con sprints 5.5.2 y 5.5.4). v_compras_pendientes y v_reposicion_explain
-- recreadas en CASCADE sin cambios funcionales.
--
-- IMPACTO MEDIDO HOY:
-- LT promedio 22 agotar: 14.00 → 5.00 (-64%)
-- safety_stock promedio: 4.64 → 4.50 (-3%)
-- Σ qty_a_comprar: 179 → 156 uds (-13%)
-- Caso testigo JSAFAB422P20S: lt 14→5, cycle 6→2, ss 4→3, rop 10→4.
--
-- AUDITORÍA POST-DEPLOY: 0 vistas/funciones en prod con el filtro viejo.
...
-- =============================================================================
-- v_safety_stock — supplier_lt CTE alineada a doctrina post-revert
-- =============================================================================
...
WITH demand_stats AS (
  SELECT si.sku_origen,
         CASE
           WHEN si.es_quiebre_proveedor = true AND si.vel_pre_quiebre IS NOT NULL AND si.vel_pre_quiebre > 0::numeric AND si.vel_pre_quiebre > (COALESCE(si.vel_ponderada, 0::numeric) * 2::numeric) THEN si.vel_pre_quiebre
  ...
  WHERE COALESCE(si.vel_ponderada, 0::numeric) > 0::numeric
     OR si.es_quiebre_proveedor = true AND COALESCE(si.vel_pre_quiebre, 0::numeric) > 0::numeric
), supplier_lt AS (
  SELECT p.sku,
         p.proveedor_id,
  ...
  -- Sprint 5.5.5: alineado con doctrina post-revert agotar (sprint 5.5.2).
  ...
  SELECT snp.sku_origen, snp.node_id,
         ...
         snp.target_dias_flex, snp.action, snp.xyz_confidence,
         snp.seasonal_match_source, snp.policy_status, snp.flex_priority,
         snp.tendencia, snp.promocion_activa, snp.promocion_motivo
...
SELECT pe.sku_origen, pe.node_id,
  ...
  ROUND(
    CASE
      WHEN COALESCE(slt.sigma_lt, 0::numeric) < 2::numeric THEN pe.z_value * (d.sigma_sem / sqrt(7.0)) * sqrt(COALESCE(slt.lt_dias_avg, 14::numeric)) * 1.075
      ELSE pe.z_value * sqrt(COALESCE(slt.lt_dias_avg, 14::numeric) * power(d.sigma_sem / sqrt(7.0), 2::numeric) + power(d.d_avg_sem / 7.0, 2::numeric) * power(COALESCE(slt.sigma_lt, 2::numeric), 2::numeric)) * 1.075
    END
  )::integer AS safety_stock,
  ROUND(d.d_avg_sem / 7.0 * COALESCE(slt.lt_dias_avg, 14::numeric))::integer AS cycle_stock,
  ROUND(d.d_avg_sem / 7.0 * COALESCE(slt.lt_dias_avg, 14::numeric) + pe.z_value * (d.sigma_sem / sqrt(7.0)) * sqrt(COALESCE(slt.lt_dias_avg, 14::numeric)))::integer AS reorder_point,
  CASE
...
WHERE pe.policy_status = 'active'::text AND pe.action <> 'no_reorder'::policy_action_enum;

COMMENT ON VIEW v_safety_stock IS
  'Sprint 5.5.5 (revert agotar supplier_lt, 2026-05-04 PM): CTE supplier_lt alineada
   a doctrina post-revert. Antes excluía agotar -> 22 SKUs usaban LT default 14 en
   lugar del LT real del proveedor (5d Idetex). Cierra el barrido del filtro espejo
   (4 superficies: RPC sync-templates [5.5.2] + v_trend_detection [5.5.4] + v_safety_stock [5.5.5]).';

-- =============================================================================
-- v_compras_pendientes (rebuild por CASCADE — sin cambios funcionales vs Sprint 5.5 v3)
-- =============================================================================
...
SELECT ss.sku_origen, p.nombre,
  ...
  GREATEST(0::numeric, (ss.safety_stock + COALESCE(pf.pre_full_target, 0) + COALESCE(rf.reserva_flex_target, 0))::numeric - COALESCE(st.stock_total, 0::numeric) - COALESCE(et.in_transit_total, 0::numeric)) AS qty_a_comprar,
  CASE
    WHEN p.costo_promedio IS NULL OR p.costo_promedio = 0::numeric THEN NULL::numeric
    ELSE GREATEST(0::numeric, (ss.safety_stock + COALESCE(pf.pre_full_target, 0) + COALESCE(rf.reserva_flex_target, 0))::numeric - COALESCE(st.stock_total, 0::numeric) - COALESCE(et.in_transit_total, 0::numeric)) * p.costo_promedio
  END AS clp_estimado,
  ...
  CASE
    WHEN (COALESCE(st.stock_total, 0::numeric) + COALESCE(et.in_transit_total, 0::numeric)) < (ss.safety_stock + COALESCE(pf.pre_full_target, 0) + COALESCE(rf.reserva_flex_target, 0))::numeric THEN true
    ELSE false
  END AS bajo_rop,
...
WHERE ss.node_id = 'bodega_central'::text
  AND (COALESCE(st.stock_total, 0::numeric) + COALESCE(et.in_transit_total, 0::numeric)) < (ss.safety_stock + COALESCE(pf.pre_full_target, 0) + COALESCE(rf.reserva_flex_target, 0))::numeric;

COMMENT ON VIEW v_compras_pendientes IS
  'Recreada Sprint 5.5.5 por CASCADE. Sin cambios funcionales — definición exacta de Sprint 5.5 v3.';

-- =============================================================================
-- v_reposicion_explain (rebuild por CASCADE — definición exacta sprint 5.5.4)
-- =============================================================================
...
SELECT vsf.sku_origen,
  ...
  COALESCE(vsn_b.qty_on_hand - COALESCE(vsn_b.qty_reserved, 0), 0::numeric) + COALESCE(vsn_f.qty_on_hand, 0::numeric) AS stock_total,
  COALESCE(its.in_transit_total, 0::numeric) AS in_transit_bodega,
  CASE WHEN COALESCE(vsn_b.qty_on_hand - COALESCE(vsn_b.qty_reserved, 0), 0::numeric) <= 0::numeric THEN 'EN_QUIEBRE'::text ELSE 'OK'::text END AS quiebre_bodega_estado,
  ...
  CASE WHEN COALESCE(vsn_b.qty_on_hand - COALESCE(vsn_b.qty_reserved, 0), 0::numeric) <= 0::numeric AND COALESCE(vsn_f.qty_on_hand, 0::numeric) <= 0::numeric
       ...
       WHEN COALESCE(vsn_b.qty_on_hand - COALESCE(vsn_b.qty_reserved, 0), 0::numeric) > 0::numeric AND COALESCE(vsn_f.qty_on_hand, 0::numeric) <= 0::numeric
       THEN ('Full quebrado: armar envío Bodega->Full hoy. Tenes '::text || COALESCE(vsn_b.qty_on_hand - COALESCE(vsn_b.qty_reserved, 0), 0::numeric)::text) || ' unidades disponibles'::text
  ...
  p.costo_promedio,
  snp.manual_override, snp.policy_status, snp.seasonal_match_source,
...
COMMENT ON VIEW v_reposicion_explain IS
  'Recreada Sprint 5.5.5 por CASCADE. Sin cambios funcionales — definición exacta de Sprint 5.5.4.';