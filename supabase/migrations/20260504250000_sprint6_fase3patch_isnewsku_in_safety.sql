-- =============================================================================
-- Sprint 6 — Fase 3 patch: incluir is_new_sku en v_safety_stock.demand_stats
-- =============================================================================
-- batch:20260504-sprint-6-cerrar-gap | sprint:6 | fase:3-patch
--
-- Bug: la migration anterior dejó la columna is_new_sku populada y la accion
-- correcta en sku_node_policy, pero v_compras_pendientes filtra (via JOIN
-- v_safety_stock) por demand_stats.WHERE vel_ponderada>0 OR quiebre. is_new_sku
-- tiene vel=0 por definición, entonces nunca aparece en safety_stock → no
-- aparece en compras_pendientes → mandar_full_uds nunca se computa.
--
-- Fix: ampliar demand_stats.WHERE para incluir SKUs con flag is_new_sku=true
-- en cualquier nodo. d_avg_sem queda en 0 (porque vel=0), safety/cycle/rop=0.
-- v_compras_pendientes filtra por (bajo_rop OR is_new_sku) y ahora sí los
-- expone con mandar_full_uds derivado.
-- =============================================================================
...
WITH demand_stats AS (
    SELECT si.sku_origen,
     ...
     WHERE COALESCE(si.vel_ponderada, 0::numeric) > 0::numeric
        ...
        -- Sprint 6 Fase 3 patch: incluir is_new_sku para que mandar_full_uds
        -- pueda computarse en v_compras_pendientes.
        OR EXISTS (
            SELECT 1 FROM sku_node_policy snp
    ...
    SELECT snp.sku_origen, snp.node_id,
           ...
           snp.seasonal_match_source, snp.policy_status,
           snp.flex_priority, snp.tendencia,
           snp.promocion_activa, snp.promocion_motivo
...
'Sprint 6 Fase 3 patch (2026-05-04 PM): demand_stats amplifica WHERE para
 incluir SKUs flagged is_new_sku (rama EXISTS sku_node_policy). Permite que
 v_compras_pendientes los exponga con mandar_full_uds.';

-- v_compras_pendientes (idéntica a Fase 3)
...
'Sprint 6 Fase 3 (2026-05-04 PM, patched): expone qty_raw, qty_a_comprar
 (redondeado a inner_pack), delta_pack, is_new_sku, mandar_full_uds.';

-- v_reposicion_explain (idéntica Fase 3)
