-- Sprint 6 Fase 3 patch 2: bypass filtro action='no_reorder' para is_new_sku
-- Los SKUs flagged is_new_sku caen en CZ con action='no_reorder' (template)
-- pero la doctrina motor viejo NUEVO/MANDAR_FULL es ortogonal — siempre se exponen.
...
SELECT pe.sku_origen, pe.node_id,
       ...
       round(
           CASE
               ...
               THEN COALESCE(pe.z_value, 0) * (d.sigma_sem / sqrt(7.0)) * sqrt(COALESCE(slt.lt_dias_avg, 14::numeric)) * 1.075
               ELSE COALESCE(pe.z_value, 0) * sqrt(COALESCE(slt.lt_dias_avg, 14::numeric) * power(d.sigma_sem / sqrt(7.0), 2::numeric)
       ...
       round(d.d_avg_sem / 7.0 * COALESCE(slt.lt_dias_avg, 14::numeric)
             + COALESCE(pe.z_value, 0) * (d.sigma_sem / sqrt(7.0)) * sqrt(COALESCE(slt.lt_dias_avg, 14::numeric)))::integer AS reorder_point,
...
SELECT ss.sku_origen, p.nombre,
       ...
       CASE WHEN COALESCE(pb.is_new_sku, false)
