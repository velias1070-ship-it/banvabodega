-- Sprint 7 Fase 1.2 (refinamiento) — extender bypass al caso blocked_no_cost
-- batch:20260505-sprint-7-fase12 | sprint:7 | fase:1.2
--
-- Los 4 SKUs huérfanos (JSCNAE190P15W, JSCNAE190P20W, SPAFE30E10W26,
-- SPAFE40O15W26) caen en blocked_no_cost (productos.costo_promedio=0)
-- antes de chequear abc/xyz. Para SKUs is_new_sku el costo aún no se
-- alimenta porque no hay recepción ni venta. Bypass: si is_new_sku,
-- asignar cell='BY' y status='active' incluso sin costo.
...
BEGIN
  ...
  -- Sprint 7 Fase 1.2: cell default + bypass blocked_* para is_new_sku.
  -- SKUs nuevos sin costo/historia se ponen 'active' con cell default
  -- ('BY' nuevos, 'CY' resto) para que el pipeline los procese.
