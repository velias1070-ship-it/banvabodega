-- Sprint 9 P2.bis — Sincronizar accion canónica con qty_a_comprar + liquidacion_accion
-- batch:20260506-sprint-9-p2-bis | sprint:9 | milestone:sprint-9-cell-sync-canon
-- tags: hotfix:accion-canonica-rules-order
--
-- 2 fixes correlacionados:
--
-- (1a) Override AGOTADO_SIN_PROVEEDOR + qty_a_comprar=0
--      En v_compras_pendientes, cuando si.tiene_stock_prov=false:
--      - qty_raw queda intacto (auditoría del cálculo del motor).
--      - qty_a_comprar = 0 (no sugerir comprar al que no tiene).
--      - clp_estimado = 0 acorde.
--      Doctrina: "no le pidas al proveedor que no tiene stock".
--
-- (1b) Override LIQUIDACION + qty_a_comprar=0 (simetría doctrinal con AGOTADO)
--      Cuando snp.liquidacion_accion ≠ 'monitorear'/'no_aplica':
--      - qty_raw queda intacto.
--      - qty_a_comprar = 0 (no comprás más cuando estás liquidando).
--      - clp_estimado = 0 acorde.
--      Doctrina: "si liquidás, no comprás más" (extensión natural del override AGOTADO).
--
-- (2) Columna `accion` recalculada con 10 reglas canónicas (primera que matchea)
--     En v_reposicion_explain, reemplazar `si.accion` (pass-through motor viejo)
--     por CASE expression con las 10 reglas:
--
--       Rule 1.  URGENTE              — vel>0 AND (cob<7d cruda OR dio<14)
--       Rule 2.  AGOTADO_SIN_PROVEEDOR — tiene_stock_prov=false AND bajo_rop
--       Rule 3.  LIQUIDACION          — liquidacion_accion ≠ monitorear/no_aplica
--       Rule 4.  MANDAR_FULL          — mandar_full_uds > 0
--       Rule 5.  PEDIR_PROVEEDOR      — qty_a_comprar > 0 AND tiene_stock_prov=true
--       Rule 6.  EN_TRANSITO          — in_transit_bodega > 0
--       Rule 7.  PLANIFICAR           — bajo_rop=true
--       Rule 8.  NUEVO                — is_new_sku=true
--       Rule 9.  EXCESO               — dio > 60 (umbral motor viejo)
--       Rule 10. OK                   — default
--
--     `si.accion` queda expuesta como `accion_motor_viejo` para auditoría.
--
-- Tests: T33, T34 en tests/sql/regression_sprint9_combined.sql.
-- Sprint 9 P1 invariantes T28, T29 + Camino A T30, T31, T32 deben seguir
-- pasando a 0 post-fix.
...
-- ───────────────────────────────────────────────────────────────────────
-- v_compras_pendientes con override AGOTADO+qty=0 (P2.bis)
-- ───────────────────────────────────────────────────────────────────────
...
WITH stock_total_por_sku AS (
  ...
  -- P2.bis: tiene_stock_prov para override AGOTADO+qty=0
  SELECT si.sku_origen,
    COALESCE(si.tiene_stock_prov, true) AS tiene_stock_prov
...
SELECT ss.sku_origen, p.nombre, ss.cell, ss.cell_original, ss.tendencia,
  ...
  -- P2.bis: override AGOTADO+qty=0 + override LIQUIDACION+qty=0
  CASE
  ...
  CASE
    ...
    WHEN pb.liquidacion_accion IS NOT NULL
      AND pb.liquidacion_accion::text NOT IN ('monitorear', 'no_aplica') THEN 0::numeric
    WHEN COALESCE(ip.inner_pack, 1) > 1 AND qc.qty_raw > 0::numeric
      THEN ceil(qc.qty_raw / ip.inner_pack::numeric) * ip.inner_pack::numeric - qc.qty_raw
    ELSE 0::numeric
  END AS delta_pack,
  -- clp_estimado coherente con overrides
  CASE
    WHEN COALESCE(tsp.tiene_stock_prov, true) = false THEN 0::numeric
    WHEN pb.liquidacion_accion IS NOT NULL
      AND pb.liquidacion_accion::text NOT IN ('monitorear', 'no_aplica') THEN 0::numeric
    ...
    WHEN COALESCE(ip.inner_pack, 1) > 1 AND qc.qty_raw > 0::numeric
      THEN ceil(qc.qty_raw / ip.inner_pack::numeric) * ip.inner_pack::numeric * p.costo_promedio
  ...
  -- P2.bis: tiene_stock_prov expuesto para v_reposicion_explain
...
-- ───────────────────────────────────────────────────────────────────────
-- v_reposicion_explain con accion canónica (10 reglas)
-- ───────────────────────────────────────────────────────────────────────
...
SELECT vsf.sku_origen, p.nombre, p.categoria, p.proveedor_id, pr.nombre_canonico AS proveedor_nombre,
       ...
       -- ─────────────────────────────────────────────────────────────────
       -- P2.bis: accion canónica con 10 reglas (primera que matchea gana)
       -- ─────────────────────────────────────────────────────────────────
       CASE
         -- Rule 1: URGENTE — vel>0 AND (cob<7d cruda OR dio<14)
         WHEN vsf.d_avg_dia > 0::numeric AND (
           (COALESCE(vsn_b.qty_on_hand - COALESCE(vsn_b.qty_reserved, 0::numeric), 0::numeric)
         ...
         WHEN COALESCE(si.tiene_stock_prov, true) = false
           ...
           THEN 'AGOTADO_SIN_PROVEEDOR'::text
         -- Rule 3: LIQUIDACION — liquidacion_accion ≠ monitorear/no_aplica
         WHEN snp.liquidacion_accion IS NOT NULL
           AND snp.liquidacion_accion::text NOT IN ('monitorear', 'no_aplica')
           THEN 'LIQUIDACION'::text
         -- Rule 4: MANDAR_FULL — mandar_full_uds > 0
         WHEN COALESCE(vcp.mandar_full_uds, 0::numeric) > 0::numeric
           THEN 'MANDAR_FULL'::text
         -- Rule 5: PEDIR_PROVEEDOR — qty_a_comprar > 0 AND tiene_stock_prov=true
         WHEN COALESCE(vcp.qty_a_comprar, 0::numeric) > 0::numeric
           AND COALESCE(si.tiene_stock_prov, true) = true
           THEN 'PEDIR_PROVEEDOR'::text
         -- Rule 6: EN_TRANSITO — in_transit_bodega > 0
         WHEN COALESCE(its.in_transit_total, 0::numeric) > 0::numeric
           THEN 'EN_TRANSITO'::text
         -- Rule 7: PLANIFICAR — bajo_rop=true
         WHEN COALESCE(vcp.bajo_rop, false) = true
           THEN 'PLANIFICAR'::text
         -- Rule 8: NUEVO — is_new_sku=true
         WHEN COALESCE(vcp.is_new_sku, false) = true
           THEN 'NUEVO'::text
         -- Rule 9: EXCESO — dio > 60 (umbral motor viejo)
         WHEN COALESCE(vsf.dio, 0::numeric) > 60::numeric
           THEN 'EXCESO'::text
         -- Rule 10: OK default
         ELSE 'OK'::text
       END AS accion,
...
-- ───────────────────────────────────────────────────────────────────────
-- v_sku_explanation (verbatim — sin cambios de doctrina)
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- COMMENTS — documentar cambio de doctrina
-- ───────────────────────────────────────────────────────────────────────
COMMENT ON COLUMN v_compras_pendientes.qty_a_comprar IS
  'Sprint 9 P2.bis: si tiene_stock_prov=false → qty=0. Si liquidacion_accion ≠ monitorear/no_aplica → qty=0 (simetría con AGOTADO). qty_raw queda intacto para auditoría.';

COMMENT ON COLUMN v_compras_pendientes.tiene_stock_prov IS
  'Sprint 9 P2.bis: pass-through de sku_intelligence.tiene_stock_prov para alimentar el override AGOTADO+qty=0 y la regla de accion canónica.';

COMMENT ON COLUMN v_reposicion_explain.accion IS
  'Sprint 9 P2.bis: accion canónica calculada con 10 reglas (primera que matchea gana): URGENTE > AGOTADO_SIN_PROVEEDOR > LIQUIDACION > MANDAR_FULL > PEDIR_PROVEEDOR > EN_TRANSITO > PLANIFICAR > NUEVO > EXCESO > OK. Reemplaza el pass-through de sku_intelligence.accion (motor viejo) — ahora expuesto como accion_motor_viejo para auditoría.';

COMMENT ON COLUMN v_reposicion_explain.accion_motor_viejo IS
  'Sprint 9 P2.bis: pass-through de sku_intelligence.accion (motor viejo, intelligence.ts paso 15). Conservado para auditoría/comparación con la accion canónica.';
