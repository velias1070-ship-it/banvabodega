...
- **Autoheal `composicion_venta` trivial** — `syncStockFull` tiene 2 pases: (5b) inline sobre `mapped` (items que ML devolvió en esa corrida) + (5c) `autohealComposicionExtendido` sobre TODO `ml_items_map.activo=true` con `sku=sku_venta` (PR6c, 2026-04-21). El 5c rescata composiciones triviales huérfanas para SKUs que ML API no lista en fulfillment (paginación, filtros de estado). No crea productos nuevos, solo rescata composición trivial para SKUs con producto ya existente + upsert con `onConflict: "sku_venta,sku_origen"`. Previene bug histórico donde 26 SKUs quedaron invisibles al motor (vel_ponderada=0 aunque vendían) durante semanas.
- **Bodega Flex BANVA en ML:** BANVA tiene **un solo warehouse Flex** registrado, `store_id=73722087` con `network_node_id=CLP19538063212` (formato `CL + P + {seller_id=1953806321} + 2`). Es un `seller_warehouse` (Flex multi-origen), gestionado por `updateFlexStock` con `stockType="seller_warehouse"` cuando `getSellerStockType` lo detecta (`ml.ts:1257-1303`). Los items Full (tipo `meli_facility`) son stock ML no controlable. Investigación detallada en PR6b-pivot-I.
